...
The two knowledge base tools answer FL3XX and other "how do I" questions from the for-Support KB (`.ai/decisions.md` 003). They need two read-only routes on for-Support that do not exist yet; the contract is `docs/common-proposed/for-support-kb-read-api.md`. `docs/common-proposed/` holds proposals this repo writes for other ForIT repos (cross-project writes are not made from here).
...
| `WingmanToolCatalog.swift` | ~430 | The static allow-list of gateway tools described to the model (five: tickets, draft note, flights, KB search, KB article), their JSON schemas, the narrowing to the gateway's `tools/list` (`modelToolDefinitions(exposedByGateway:)`), the app-side argument policy (`prepareCall`: draft-note prefix and author label, forced KB tenant, key whitelisting, limit clamping) and result condensing (ticket lists, article search results, article body cap). |
