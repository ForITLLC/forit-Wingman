...
- **Sign-in**: Microsoft Entra authorization-code + PKCE in the system browser (`WingmanEntraSignInManager.swift`), custom URL scheme `msauth.io.forit.wingman://auth`, refresh token in the login keychain. Since 2026-09-05 (Ben: "every fucking time I come back, there's like a million unlock my keychain requests, even though I click like always allow"; decision 014) the keychain item is created with an access list that names no trusted application (`SecAccessCreate` with a nil list, set as `kSecAttrAccess`), so no launch shows the "Wingman wants to use your confidential information" dialog: securityd keys silent access to the reader's code signature, the interim self-signed certificate is not one it trusts, so every launch of a build prompted and "Always Allow" never carried over (securityd `kcacl` lines on Ben's Mac, 2026-09-05). The read of that item at launch (`restoreSessionIfPossible`) runs off the main thread in a detached task, so a dialog an older build's item still raises never blocks the panel or the Sparkle check again. A Mac with an item from before this change sees that dialog one last time; the next sign-in or token refresh rewrites the item with the open list. The trade-off is that any process running as the signed-in user could read the refresh token without a dialog, the same posture as a token file in the user's own Library; move to the data protection keychain once a Developer ID gives the app a Team ID. The id_token is the bearer for the relay; a gateway access token (`api://861db494-…/access_as_user`, client `acc81527-…`, see for-mcp `docs/native-clients.md`) is minted for the for-mcp tools layer.
...
| `CompanionManager.swift` | ~1490 | Central state machine. Owns the sign-in manager, the vocabulary store, the usage sharing preference, dictation, shortcut monitoring, screen capture, the relay chat client, the gateway tool client, TTS, and overlay management. Tracks voice state (idle/listening/processing/responding), conversation history, model selection, cursor visibility, the gateway access banner, the last cited knowledge base article and the cached gateway `tools/list`. Coordinates the full push-to-talk → screenshot → Claude (tool loop) → TTS → pointing pipeline and refuses it while signed out. The voice system prompt, including the ForIT support and knowledge base section, lives here. Feeds the sentence splitter from the stream callback, warms the relay and tool list on key-down, and fetches ForIT Support's vocabulary in the background after sign-in and hourly. |
...
| `CompanionPanelView.swift` | ~1070 | SwiftUI panel content for the menu bar dropdown. Shows the ForIT mark and companion status, the account row (sign in / signed-in user / sign out), the gateway access banner, the usage sharing notice until it is acknowledged and the "Share usage with ForIT" switch after, push-to-talk instructions, model picker from the relay allow-list, the Source row (the ForIT Support article the last answer was read from, with an Open button), the collapsible read-only Vocabulary list with its source line, permissions UI, and quit button. Dark aesthetic using `DS` design system. |
...
| `WingmanSignInSupport.swift` | ~205 | PKCE helpers, id_token claim reader (display only, no local signature check), keychain store (`WingmanKeychainStore`, `Sendable`; saves the refresh token with an access list open to every application of this user so no launch prompts, decision 014). |
...
| `WingmanVocabulary.swift` | ~365 | The vocabulary: a term is its canonical spelling (`FL3XX`), its spoken forms (`Flex`) and a one-line meaning. Pure functions for the four uses (Apple Speech keyterms, whole-word transcript rewrite to the canonical spelling, the prompt section, canonical-to-spoken rewrite before text-to-speech), `containsTerm` for the tool policy, the name of the gateway tool (`support_listVocabulary`) and the parser of its result, and `WingmanVocabularyStore`, the JSON file in `~/Library/Application Support/io.forit.wingman/vocabulary.json` that holds the last list ForIT Support published (built-in FL3XX seed until then; no local add or remove). |
| `WingmanToolCatalog.swift` | ~800 | The static allow-list of gateway tools described to the model (eight: tickets, draft note, flights, KB search, KB article, tenant list, tenant directory lookup, create ticket), their JSON schemas, the narrowing to the gateway's `tools/list` (`modelToolDefinitions(exposedByGateway:)`), the app-side argument policy (`prepareCall`: draft-note prefix and author label, forced KB tenant, keyword-only KB query, KB category pass-through, key whitelisting, limit clamping, and the two-call ticket filing: preview without a token, confirm only with the held preview and a spoken go-ahead, the filed-by line) and result condensing (ticket lists, article search results, article body cap, tenant and person fields, the ticket preview without its token, the created ticket's number, a send-rail hold as a fixed instruction) and `knowledgeBaseArticleCitation(fromResultText:)`, the title and url of a read article for the Source row. |
...
| `WingmanSpokenSentenceSplitter.swift` | ~150 | Pure splitter for streamed replies: releases the first sentence at its boundary, later ones once at least 60 characters are pending, never speaks from `[POINT` or from a `Source:` line onward and holds back a partial tag; `flushRemaining` at end of stream; `speakableText(ofSentence:)` is what speech gets for one sentence, with every web address removed and a Source line skipped whole. Unit-tested. |
| `DesignSystem.swift` | ~880 | Design system tokens — colors, corner radii, shared styles. All UI references `DS.Colors`, `DS.CornerRadius`, etc. |
| `WingmanAnalytics.swift` | ~190 | Local analytics wrapper: same static API upstream used for PostHog, now `os.Logger` lines only. Ordinary events are debug lines; `tool_refused`, `tool_failed`, `response_error`, `tts_error` and `vocabulary_refresh_failed` are error-level with public fields so `log show --predicate 'subsystem == "io.forit.wingman"'` can read them back; `kb_article_cited`, `usage_report_sent` and `vocabulary_refreshed` are notice-level milestones. Tool events log the tool name and a stable outcome label, never arguments or results. |
...
| `WingmanTests/WingmanSpokenSentenceSplitterTests.swift` | ~97 | Swift Testing coverage for the sentence splitter: early first sentence, batching, number periods, newlines, the pointing tag, the Source line never being spoken, web addresses left out of speech, the final flush. |
...
| `WingmanTests/WingmanVocabularyTests.swift` | ~200 | Swift Testing coverage for the vocabulary: keyterms, whole-word transcript rewrite (longest spoken form first), pronunciation rewrite, prompt section, product-name-as-tenant fallback in the tool policy, the `tool_error_http_NNN` label, the parsing of ForIT Support's list, and the store (built-in seed, an empty list keeps it, the fetched list round-trips with its fetch time, an old per-Mac file is ignored). |
| `WingmanTests/WingmanToolUseTests.swift` | ~490 | Swift Testing coverage for the tool allow-list and draft policy, the knowledge base argument policy (tenant, keywords, category) and condensing, the article citation, the `tools/list` narrowing and parsing, the SSE `tool_use` parser, and gateway response / refusal parsing. |
| `WingmanTests/WingmanTicketFilingTests.swift` | ~248 | Swift Testing coverage for ticket filing: the consent vocabulary, the three tools' access levels and argument policy, the preview call (lower-cased email, filed-by line once, priority default), the confirming call (held preview resent with token and words; refused without a preview, with a stale one, on a bare yes, declined on a no), and the condensing of previews, created tickets, holds, tenants and people.
