...
- **Colours**: the accent and the overlay cursor are the ForIT brand cyan `#8EDCEF` with brand navy `#142F43` as the text on it (`DS.Colors.foritCyan`, `foritNavy`, `overlayCursorColor`; Ben, 2026-09-05: "make the blue for Wingman one of the ForIT colors"). The upstream Tailwind blue scale is still declared but no longer drives the accent. The app icon and the menu bar glyph (2026-09-05, Ben: "should we make the icon on this, like the ForIT icon, with an arrow on top") are the ForIT hexagon mark with the Wingman pointer landing on its lower-right corner: the icon is a navy tile with the white-and-cyan mark (`Assets.xcassets/AppIcon.appiconset`, 16 to 1024 px), the menu bar glyph is the outline and pointer as a template image (`Assets.xcassets/MenuBarIcon.imageset`, 1x/2x/3x, loaded by `MenuBarPanelManager.makeWingmanMenuBarIcon()` with the old drawn triangle as the fallback). Both are rendered with `rsvg-convert` from the sources in `docs/brand/`; the mark's geometry is the ForIT logo SVG with the viewBox `46 183 164 180`. The pointer is built like the mark, a cyan face inside a line (Ben, 2026-09-05: "make the arrow the forit shape? add a line around it … light is sometimes hard to see"): on the icon the line is white with a navy gap so it cuts out of the hexagon, on the menu bar glyph a transparent gap is knocked out around it, and the live overlay pointer (`BlueCursorView` in `OverlayWindow.swift`) is the cyan face with a navy line (`DS.Colors.overlayCursorOutlineColor`) and a white halo (`overlayCursorHaloColor`) so it reads on light and dark screens alike. The small cyan bubbles beside the pointer use `DS.Colors.textOnAccent` (navy) rather than white text.
