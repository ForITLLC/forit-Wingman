...
| "Which clients do we support?" / "Is Planet Nine a client?" | `support_listTenants` | Viewer | none |
| "Find `<person>` at `<client>`" (to get the email a ticket needs) | `support_listInventoryUsers` | Viewer | none. Always a search for one person (name or email); the app never sends a whole-directory listing. |
| "Put in a ticket for `<person>` at `<client>`" | `support_listTenants`, `support_listInventoryUsers`, then `support_createTicket` twice | Operator | **Creates a ticket and for-Support emails the requester**, so it is the one action with a spoken gate (decision 011). First call: a preview, nothing created; the app holds the token and the exact arguments for ten minutes and the model reads the preview back. Second call, in a later turn: filed only if what the person said this turn passes `WingmanSpokenConsent` ("go ahead", "create it", "do it", "proceed"; "no" / "don't" / "cancel" discard the preview; a bare "yes" or a question files nothing), and then with the previewed arguments, the token and the words as `consent`. for-Support's send rail judges the same words again and answers 409 `send_rail_hold` if it disagrees; until for-Support declares `consent` in its OpenAPI schema the gateway drops the field and every live create is held (`docs/common-proposed/for-support-ticket-create-consent.md`). The description carries `Filed by <name> (<email>) with Wingman.`; for-Support records the create under `automation@forit.io`. |
